...
void keypad_read(lv_indev_drv_t *indev_drv, lv_indev_data_t *data)
{
    ...
    if (act_key != 0)
    {
        ...
        case 6:
            act_key = LV_KEY_DEL;
            lv_event_send(del_btn, LV_EVENT_CLICKED, NULL); //发送事件给del_btn
            break;
        case 7:
            act_key = LV_KEY_ESC;                           //退出键
                                                            //						lv_event_send(add_btn, LV_EVENT_CLICKED, NULL);//发送事件给add_btn
            lv_event_send(esc_btn, LV_EVENT_CLICKED, NULL); //发送事件给esc_btn
            break;
        case 8:
            act_key = LV_KEY_END;
            lv_event_send(run_btn, LV_EVENT_CLICKED, NULL); //发送事件给run_btn
            break;
        }
